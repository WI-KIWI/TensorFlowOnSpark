  ...
  if tf.test.is_built_with_cuda():
    ...
    while not gpu_initialized:
      try:
        ...
        if rdma:
          server = tf.train.Server(cluster, ctx.job_name, ctx.task_index, protocol="grpc_rdma", config=tf.ConfigProto(gpu_options=gpu_options))
        else:
          server = tf.train.Server(cluster, ctx.job_name, ctx.task_index, config=tf.ConfigProto(gpu_options=gpu_options))
